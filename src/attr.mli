...
val create : ?here:Stdlib.Lexing.position -> string -> string -> t
...
val bool_property : ?here:Stdlib.Lexing.position -> string -> bool -> t

(** [property name value] creates a property with a generic value *)
val property : ?here:Stdlib.Lexing.position -> string -> Js.Unsafe.any -> t
...
val many_without_merge : ?here:Stdlib.Lexing.position -> t list -> t
...
val autofocus : ?here:Stdlib.Lexing.position -> bool -> t
val checked : t
val checked_prop : ?here:Stdlib.Lexing.position -> bool -> t
val class_ : string -> t
...
val inert : t
val allow : ?here:Stdlib.Lexing.position -> string -> t
val for_ : ?here:Stdlib.Lexing.position -> string -> t
val label : ?here:Stdlib.Lexing.position -> string -> t
val href : ?here:Stdlib.Lexing.position -> string -> t
val rel : ?here:Stdlib.Lexing.position -> string -> t
val target : ?here:Stdlib.Lexing.position -> string -> t
val id : ?here:Stdlib.Lexing.position -> string -> t
val name : ?here:Stdlib.Lexing.position -> string -> t
val placeholder : ?here:Stdlib.Lexing.position -> string -> t
val role : ?here:Stdlib.Lexing.position -> string -> t
val selected : t
val hidden : t
val readonly : t
val style : Css_gen.t -> t
val min : ?here:Stdlib.Lexing.position -> float -> t
val max : ?here:Stdlib.Lexing.position -> float -> t
val min_date : ?here:Stdlib.Lexing.position -> Date.t -> t
val max_date : ?here:Stdlib.Lexing.position -> Date.t -> t
...
val min_date_time : ?here:Stdlib.Lexing.position -> Date.t -> t

(** Similar to [min_date_time], but sets the maximum instead. *)
val max_date_time : ?here:Stdlib.Lexing.position -> Date.t -> t

val colspan : ?here:Stdlib.Lexing.position -> int -> t
val rowspan : ?here:Stdlib.Lexing.position -> int -> t
...
val on_transitionend : (Dom_html.transitionEvent Js.t -> unit Effect.t) -> t
...
module Multi : sig
  ...
  val merge_classes_and_styles : ?here:Stdlib.Lexing.position -> t -> t
  ...
  val map_style : ?here:Stdlib.Lexing.position -> t -> f:(Css_gen.t -> Css_gen.t) -> t

  val add_style : t -> Css_gen.t -> t
  val add_class : ?here:Stdlib.Lexing.position -> t -> string -> t
end
